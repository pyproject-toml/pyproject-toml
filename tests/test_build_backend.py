...
SETUP_SCRIPT_STUB = "__import__('pyproject_toml').setup()"
...
defns = [
    {  # simple setup.py script
        "setup.py": DALS(
            """
            __import__('pyproject_toml').setup(
                name='foo',
                version='0.0.0',
                py_modules=['hello'],
                setup_requires=['six'],
    ...
    {  # setup.cfg and setup.py
        ...
        "setup.py": "__import__('pyproject_toml').setup()",
        "hello.py": DALS(
            """
        def run():
            print('hello')
...
class TestBuildMetaBackend:
    ...
    def test_build_with_existing_file_present(self, build_type, tmpdir_cwd):
        ...
        files = {
            "setup.py": "from pyproject_toml import setup\nsetup()",
            "VERSION": "0.0.1",
            "setup.cfg": DALS(
                """
                [metadata]
...
def test_sys_exit_0_in_setuppy(monkeypatch, tmp_path):
    ...
    backend = BuildBackend(backend_name="pyproject_toml.build_system.build_backend")
